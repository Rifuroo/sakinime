...
class AnimeService {
    ...
    if (data is Map) {
      ...
      if (data['success'] == true && data['data'] is Map) {
        ...
        if (innerData['response'] is List) {
          ...
          if (kDebugMode)
            print('   📦 Found: data.response structure (new API)');
    ...
    try {
      ...
      if (response != null && response.statusCode == 200) {
        ...
        if (data is Map && data['success'] == true && data['data'] is Map) {
          ...
          if (kDebugMode)
            print(
                '✅ Home loaded: ongoing=${ongoing.length}, complete=${complete.length}');
  ...
  Future<Map<String, dynamic>> _fetchDataWithPagination(String endpoint,
      {int page = 1,
      String debugLabel = '',
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (kDebugMode)
          print(
              '✅ Loaded $label: ${list.length} anime | page=$currentPage hasNext=$hasNextPage');
  ...
  Future<List<Anime>> _fetchList(String endpoint,
      {int page = 1, String debugLabel = ''}) async {
    final result = await _fetchDataWithPagination(endpoint,
        page: page, debugLabel: debugLabel);
    return result['animes'] as List<Anime>;
  }

  // ✅ ANIME LISTS - New API endpoints with PAGINATION versions
  Future<Map<String, dynamic>> getRecentAnimeWithPagination(
          {int page = 1}) async =>
      _fetchDataWithPagination('/animes/recently-updated',
          page: page, debugLabel: 'recently-updated');

  Future<List<Anime>> getRecentAnime({int page = 1}) async =>
      _fetchList('/animes/recently-updated',
          page: page, debugLabel: 'recently-updated');

  Future<Map<String, dynamic>> getRecentAddedWithPagination(
          {int page = 1}) async =>
      _fetchDataWithPagination('/animes/recently-added',
          page: page, debugLabel: 'recently-added');

  Future<List<Anime>> getRecentAdded({int page = 1}) async =>
      _fetchList('/animes/recently-added',
          page: page, debugLabel: 'recently-added');

  Future<Map<String, dynamic>> getTopAiringWithPagination(
          {int page = 1}) async =>
      _fetchDataWithPagination('/animes/top-airing',
          page: page, debugLabel: 'top-airing');

  Future<List<Anime>> getTopAiring({int page = 1}) async =>
      _fetchList('/animes/top-airing', page: page, debugLabel: 'top-airing');

  Future<Map<String, dynamic>> getMostPopularWithPagination(
          {int page = 1}) async =>
      _fetchDataWithPagination('/animes/most-popular',
          page: page, debugLabel: 'most-popular');

  Future<List<Anime>> getMostPopular({int page = 1}) async =>
      _fetchList('/animes/most-popular',
          page: page, debugLabel: 'most-popular');

  Future<Map<String, dynamic>> getMostFavoriteWithPagination(
          {int page = 1}) async =>
      _fetchDataWithPagination('/animes/most-favorite',
          page: page, debugLabel: 'most-favorite');

  Future<List<Anime>> getMostFavorite({int page = 1}) async =>
      _fetchList('/animes/most-favorite',
          page: page, debugLabel: 'most-favorite');

  Future<Map<String, dynamic>> getOngoingAnimeWithPagination(
          {int page = 1, String order = 'popular'}) async =>
      _fetchDataWithPagination('/animes/top-airing',
          page: page, debugLabel: 'top-airing');

  Future<List<Anime>> getOngoingAnime(
          {int page = 1, String order = 'popular'}) async =>
      _fetchList('/animes/top-airing', page: page, debugLabel: 'top-airing');

  Future<Map<String, dynamic>> getCompletedAnimeWithPagination(
          {int page = 1, String order = 'latest'}) async =>
      _fetchDataWithPagination('/animes/completed',
          page: page, debugLabel: 'completed');

  Future<List<Anime>> getCompletedAnime(
          {int page = 1, String order = 'latest'}) async =>
      _fetchList('/animes/completed', page: page, debugLabel: 'completed');

  Future<Map<String, dynamic>> getPopularAnimeWithPagination(
          {int page = 1}) async =>
      _fetchDataWithPagination('/animes/most-popular',
          page: page, debugLabel: 'most-popular');
  ...
  Future<Map<String, dynamic>> getTopUpcomingWithPagination(
          {int page = 1}) async =>
      _fetchDataWithPagination('/animes/top-upcoming',
          page: page, debugLabel: 'top-upcoming');

  Future<List<Anime>> getTopUpcoming({int page = 1}) async =>
      _fetchList('/animes/top-upcoming',
          page: page, debugLabel: 'top-upcoming');

  // ✅ TYPE-BASED LISTS with PAGINATION
  Future<Map<String, dynamic>> getMoviesWithPagination(
          {int page = 1, String order = 'update'}) async =>
      _fetchDataWithPagination('/animes/movie',
          page: page, debugLabel: 'movie');

  Future<List<Anime>> getMovies(
          {int page = 1, String order = 'update'}) async =>
  ...
  Future<Map<String, dynamic>> getSpecialsWithPagination(
          {int page = 1}) async =>
      _fetchDataWithPagination('/animes/special',
          page: page, debugLabel: 'special');
  ...
  Future<Map<String, dynamic>> getSubbedAnimeWithPagination(
          {int page = 1}) async =>
      _fetchDataWithPagination('/animes/subbed-anime',
          page: page, debugLabel: 'subbed-anime');

  Future<List<Anime>> getSubbedAnime({int page = 1}) async =>
      _fetchList('/animes/subbed-anime',
          page: page, debugLabel: 'subbed-anime');

  Future<Map<String, dynamic>> getDubbedAnimeWithPagination(
          {int page = 1}) async =>
      _fetchDataWithPagination('/animes/dubbed-anime',
          page: page, debugLabel: 'dubbed-anime');

  Future<List<Anime>> getDubbedAnime({int page = 1}) async =>
      _fetchList('/animes/dubbed-anime',
          page: page, debugLabel: 'dubbed-anime');

  // ✅ SEARCH - /search?keyword={query}&page={page} with PAGINATION
  Future<Map<String, dynamic>> searchAnimeWithPagination(String query,
      {int page = 1}) async {
    ...
    return _fetchDataWithPagination('/search',
        page: page,
        debugLabel: 'search',
        extraParams: {'keyword': trimmedQuery});
  }
  ...
  Future<List<Anime>> getSuggestions(String query) async {
    try {
      ...
      final response = await _dio
          .get('/suggestion', queryParameters: {'keyword': query.trim()});
      if (response.statusCode == 200) {
  ...
  // ✅ FILTER - /filter with pagination support
  Future<Map<String, dynamic>> filterAnimeWithPagination({
    String? keyword,
    String? genres,
    String? type,
    String? status,
    ...
    final endpoint = category != null ? '/animes/$category' : '/filter';
    ...
    return _fetchDataWithPagination(endpoint,
        page: page,
        debugLabel: 'filter-${category ?? "generic"}',
        extraParams: params);
  }

  // ✅ A-Z LIST - /animes/az-list/{letter}
  Future<List<Anime>> getAnimeAZList(String letter, {int page = 1}) async {
    return _fetchList('/animes/az-list/$letter',
        page: page, debugLabel: 'az-list-$letter');
  }
          ...
          final genres = (data['data'] as List).map<Map<String, dynamic>>((e) {
            if (e is Map) {
              return {
                'id': e['id'] ??
                    e['slug'] ??
                    e['name']?.toString().toLowerCase().replaceAll(' ', '-'),
  ...
  Future<Map<String, dynamic>> getAnimeByGenreWithPagination(String genreId,
      {int page = 1}) async {
    try {
      if (kDebugMode) print('\n📂 Fetching genre "$genreId" (page: $page)');
      final response = await _dio
          .get('/animes/genre/$genreId', queryParameters: {'page': page});
      ...
      if (response.statusCode == 200) {
        ...
        if (kDebugMode)
          print(
              '✅ Genre loaded: ${animeList.length} animes | page=$currentPage hasMore=$hasNextPage');
  ...
  Future<List<Anime>> getAnimeByProducer(String producer,
      {int page = 1}) async {
    return _fetchList('/animes/producer/$producer',
        page: page, debugLabel: 'producer-$producer');
  }
  ...
  Future<AnimeDetail?> getAnimeDetail(String animeId) async {
    try {
      ...
      if (data is Map && data['success'] == true && data['data'] is Map) {
        ...
        if (kDebugMode)
          print('📊 Anime detail keys: ${animeData.keys.toList()}');
        ...
        if (kDebugMode)
          print(
              '✅ AnimeDetail parsed: ${detail.title} | eps=${detail.episodes.length}');
        return detail;
      }
      return null;
    } catch (e) {
  ...
  Future<List<Map<String, dynamic>>> getCharacters(String animeId,
      {int page = 1}) async {
    try {
      final response = await _dio
          .get('/characters/$animeId', queryParameters: {'page': page});
      if (response.statusCode == 200) {
        final data = response.data;
        ...
        if (data is Map && data['success'] == true && data['data'] is Map) {
          ...
          if (charData['response'] is List) {
            final charList =
                List<Map<String, dynamic>>.from(charData['response']);
  ...
  Future<List<Map<String, dynamic>>> getWatch2GetherRooms(
      {String room = 'all'}) async {
    try {
      final response =
          await _dio.get('/watch2gether', queryParameters: {'room': room});
      if (response.statusCode == 200) {
        final data = response.data;
        if (data is Map && data['success'] == true && data['data'] is Map) {
  ...
  Future<Map<String, dynamic>?> getServers(String episodeId) async {
    try {
      ...
      final response =
          await _dio.get('/servers', queryParameters: {'id': cleanId});
      if (response.statusCode == 200) {
        final data = response.data;
        if (data is Map && data['success'] == true && data['data'] is Map) {
          return Map<String, dynamic>.from(data['data']);
  ...
  Future<Map<String, dynamic>?> getStream(String episodeId,
      {String type = 'sub', String server = 'hd-2'}) async {
    try {
      String cleanId = episodeId.trim();
      if (cleanId.startsWith('/')) cleanId = cleanId.substring(1);
  ...
  String getEmbedUrl(String episodeId,
      {String server = 'hd-2', String type = 'sub'}) {

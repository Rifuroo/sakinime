...
class DetailAnimeScreen extends StatefulWidget {
  ...
  final bool forceHiAnime;

  const DetailAnimeScreen(
      {super.key, required this.animeId, this.forceHiAnime = false});
...
class _DetailAnimeScreenState extends State<DetailAnimeScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      provider.fetchAnimeDetail(widget.animeId,
          forceHiAnime: widget.forceHiAnime);
    });
  }
            ...
            children: [
              CustomScrollView(
                ...
                slivers: [
                  SliverAppBar(
                    ...
                    flexibleSpace: FlexibleSpaceBar(
                      background: _buildBanner(
                          anime, provider.isLoading && anime == null),
                    ),
                  ),
                        ...
                        child: Padding(
                          ...
                          child: LayoutBuilder(builder: (context, constraints) {
                                  ...
                                  SizedBox(
                                    ...
                                    child: Column(
                                      children: [
                                        const SizedBox(
                                            height:
                                                60), // Space for floating poster
                                        if (anime != null) ...[
                                          _buildRatingBadge(anime.rating),
                                          const SizedBox(height: 16),
                                          _buildActionRow(anime,
                                              provider), // Vertical/Stack action row maybe?
                                      ...
                                      children: [
                                        ...
                                        Wrap(
                                          ...
                                          children: [
                                            ...
                                            _buildPill(
                                                '${anime?.episodes.length ?? 0} Ep'),
                                            if (anime?.status != null)
                                              _buildPill(anime!.status!,
                                                  isAiring: anime.status!
                                                      .toLowerCase()
                                                      .contains('airing')),
                                          ],
                                        ),
                            ...
                            return Column(
                              ...
                              children: [
                                const SizedBox(
                                    height: 60), // Space for floating poster
                                ...
                                if (anime == null)
                                  Row(children: [
                                    _buildShimmerBlock(height: 20, width: 60),
                                ...
                                if (characters.isNotEmpty ||
                                  ...
                                  SizedBox(
                                    height: 150,
                                    child: provider.isLoadingCharacters &&
                                            characters.isEmpty
                                        ? ListView.separated(
                  ...
                  _buildMobileEpisodeSliver(
                      anime, MediaQuery.of(context).size.width > 900),
  ...
  Widget _buildInfoGridList(AnimeDetail? anime) {
    if (anime == null) {
      return _buildShimmerBlock(
          height: 200, width: double.infinity, radius: 16);
    }
    final items = [
  ...
  Widget _buildEpisodeControls(AnimeDetail? anime) {
    ...
    return Row(
      children: [
        ...
        GestureDetector(
          onTap: () =>
              setState(() => _epSort = _epSort == 'asc' ? 'desc' : 'asc'),
          child: Container(
            height: 44,
            ...
            child: Row(
              children: [
                ...
                const SizedBox(width: 8),
                Text(_epSort == 'asc' ? 'Oldest' : 'Newest',
                    style: GoogleFonts.inter(
                        color: Colors.white,
                        fontSize: 13,
                        fontWeight: FontWeight.w600))
  ...
  Widget _buildDesktopEpisodeList(AnimeDetail? anime, BuildContext context) {
    ...
    return GridView.builder(
      ...
      itemBuilder: (context, index) =>
          _buildEpisodeGridItem(visible[index], anime, context),
    );
  }
          ...
          child: Column(
            children: List.generate(
                ...
                (index) => _buildShimmerBlock(
                    height: 80, width: double.infinity, radius: 12)),
          ),
        ),
      );
    }
    ...
    return SliverPadding(
      ...
      sliver: _isGridMode
          ...
          : SliverList(
              delegate: SliverChildBuilderDelegate(
                (context, index) =>
                    _buildEpisodeItem(visible[index], anime, context),
                childCount: visible.length,
              ),
            ),
    );
  ...
  List<Episode> _getFilteredEpisodes(AnimeDetail anime) {
    List<Episode> filtered = anime.episodes
        .where((e) =>
  ...
  Widget _buildEpisodeGridItem(
      Episode ep, AnimeDetail anime, BuildContext context) {
    return GestureDetector(
      onTap: () => Navigator.push(
          context,
          MaterialPageRoute(
      ...
      builder: (context, offset, child) {
        ...
        return Container(
          ...
          padding: const EdgeInsets.only(
              top: 30, left: 60, right: 60), // Space for back / share
          color: AppColors.background
              .withValues(alpha: opacity * 0.95), // Glass-ish
          alignment: Alignment.center,
          child: Opacity(
                ...
                GestureDetector(
                  ...
                  child: ClipRRect(
                    ...
                    child: Container(
                      ...
                      decoration: BoxDecoration(
                        color: Colors.black
                            .withValues(alpha: 0.3 + (1 - opacity) * 0.7),
                      ),
      ...
      children: [
        Expanded(
            child: _buildShimmerBlock(
                height: 50, width: double.infinity, radius: 14)),
        const SizedBox(width: 12),
